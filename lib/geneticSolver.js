...
function geneticSolver(groups, ofSize, forRounds, forbiddenPairs=[]) {
  ...
  for (const i of _.range(forRounds))  {
    console.log('Round ' + (i+1) + ': ' + JSON.stringify(rounds[i]) + ' - score: ' + roundScores[i])
  }
  console.log(weights)
  return {
    rounds,
